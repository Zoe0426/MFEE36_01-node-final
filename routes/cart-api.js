    ...
    try {
        ...
        if(updateOrderResult.affectedRows){
            ...
            <p style="font-size:18px; display:inline; font-weight:bold">訂單編號: ${CustomField1}</p>
             ...
             <a href="${process.env.HOMEFRONTEND}/cart/order-complete?orderSid=${CustomField1}link&checkoutType=${CustomField2}&memberSid=${CustomField3}">按此連結，查看明細</a>
             </div>
            ...
            transporter.sendMail(mailOptions, (error, info) => {
                ...
                if (error) {
                    ...
                    //console.log("Email sent: " + info.response);
                     res.redirect(`${process.env.HOMEFRONTEND}/cart/order-complete?orderSid=${CustomField1}&checkoutType=${CustomField2}&memberSid=${CustomField3}`);
                }
                });
...
router.get('/linepay', async(req,res)=>{
    ...
    try {
        ...
        redirectUrls: {
        confirmUrl: `${process.env.BACKEND}/cart-api/linepayResult`,
        cancelUrl: `${process.env.BACKEND}/cart-api/linepayResult`
            }
      }
    })
